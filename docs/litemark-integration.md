...
### Возврат заказа из проблемы в пул

Оператор может вернуть заказ в пул только когда тот **укомплектован полностью** — требования
те же, что у автоматической превалидации:

- **РД проставлена на каждой позиции** (`rd_name` и `rd_date_to`);
- **закреплена компания обработки** (`upd_company_name`).

Иначе перевод отклоняется с текстом, называющим конкретные позиции без документов. Проверка —
`_pool_transition_blocker()` в `crm_automated/service.py`.

Смысл в том, что в пуле не должно быть неукомплектованного заказа: внешний обработчик заберёт
его как есть, без РД упрётся на регистрации карточек, а без компании неизвестно, от какого
юрлица выставлять УПД.

При успешном переводе проставляется `prevalidated_at` — без неё заказ попал бы в пул, но остался
невидимым для выдачи: гейт `_eligible_orders_query()` требует эту отметку. Заодно сбрасываются
`problem_notified_at` и `problem_ack_at` — они относятся к прошлому циклу выдачи.

> **Номер УПД на этом шаге не требуется.** Он приходит от внешнего обработчика в `/result`
> после обработки, то есть до пула его физически быть не может. Проверяется только компания,
> от которой этот УПД будет выставлен.
