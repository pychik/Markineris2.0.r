...
def time_since_top(dt: datetime):
    def get_plural_form(number: int, time_units: tuple[str, str, str]):
        if number % 10 == 1 and number % 100 != 11:
            return time_units[0]
        elif 2 <= number % 10 <= 4 and (number % 100 < 10 or number % 100 >= 20):
            return time_units[1]
    ...
    if seconds < 60:
        second_form = get_plural_form(int(seconds) + 1, ("секунда", "секунды", "секунд"))
        return f"{int(seconds) + 1} {second_form} назад"
    elif seconds < 3600:
        minutes = -(-seconds // 60)  # Округление вверх
        minute_form = get_plural_form(int(minutes), ("минута", "минуты", "минут"))
        return f"{int(minutes)} {minute_form} назад"
    elif seconds < 86400:
        hours = -(-seconds // 3600)  # Округление вверх
