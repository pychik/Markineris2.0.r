...
function socks_perform_pos_add(async_flag, url) {
    ...
    var scsq = socks_check_sizes_quantity_valid();
...
function show_socks_pos(index, trademark, type, color, pos_quantity, box_quantity, all_quantity, sq_list, content,
...
function socks_make_research_by_article(url, csrf_token) {
    var search = $('#search_by_article_text').val();
    if (search.length >= 2) {
        socks_load_search_data(search, url, csrf_token);
    } else {
        // load_search_data()
...
function socks_load_search_data(query, url, csrf_token) {
    $.ajax({
        url: url,
...
async function async_socks_delete_pos(url, csrf, block) {
    // loadingCircle();
    const settings = {
        method: 'POST',
        headers: {
...
function socks_load_upload_table(url) {
        ...
        if (!sizeAlreadyExists) {
            sizesBlock += `
                <div class="col-md-5 border-bottom border-0 border-warning text-center mt-1" >
                    ...
                    <div class="row">
                        <div class="col-4 my-2" onclick="setQuantitySize('${size[0]}');">
                            <span style="cursor: pointer">${size[0]}</span>
                        </div>
                        <div class="col-6">
