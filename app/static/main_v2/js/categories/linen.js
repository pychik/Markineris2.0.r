...
function linen_perform_pos_add(async_flag, url){
    ...
    var scsq = linen_check_sizes_quantity_valid();
    if ((pos_form.checkValidity === true || pos_form.checkValidity()) && crd && scsq && chtn) {
        ...
        allInputs.each(function( index ) {
            ...
            // if (allInputs[index].id ==='sizeX_order' ||  allInputs[index].id === 'sizeY_order'){
            //     return
            // }
...
function show_linen_pos(index, trademark, type, color, pos_quantity, box_quantity, all_quantity, sq_list, age,
                        linen_type, content, country, edit_link, copy_link, delete_link, csrf_token){
            ...
            <div class="modal-content p-3 p-md-4">
                ...
                <div class="important-card important-card--light pt-0">
                    ...
                    <div class="important-card__item">
                        ...
                        <div class="important-card__val">${age}</div>
                    </div>
                    <div class="important-card__item">
                        <div class="important-card__prop">
                            Тип белья
                        </div>
                        <div class="important-card__val">${linen_type}</div>
                    </div>
...
function linen_make_research_by_article(url, csrf_token){
    var search = $('#search_by_article_text').val();
    if(search.length >= 2){
        linen_load_search_data(search, url, csrf_token);
    }
    else{
    // load_search_data()
...
function linen_load_search_data(query, url, csrf_token)
{
    $.ajax({
    url:url,
...
async function async_linen_delete_pos(url, csrf,block){
  ...
  function linen_load_upload_table(url)
  {
...
async function linen_update_table(page)
  {
