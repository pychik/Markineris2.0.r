...
from utilities.support import check_order_pos, preprocess_order_category, common_process_delete_order, \
    helper_delete_order_pos, user_activated, helper_linen_index, manager_forbidden, \
    helper_process_category_order, helper_preload_common
...
linen = Blueprint('linen', __name__)


@linen.route('/', defaults={'o_id': None}, methods=['GET', ])
@linen.route('/<int:o_id>/', defaults={'update_flag': None}, methods=['GET', ])
@linen.route('/<int:o_id>/<int:update_flag>', methods=['GET', ])
@login_required
@user_activated
@manager_forbidden
def index(o_id: int = None, update_flag: int = None):

    return helper_linen_index(o_id=o_id, update_flag=update_flag)


@linen.route('/<int:o_id>/copy_order/<int:p_id>', defaults={'edit_order': None}, methods=['GET', ])
@linen.route('/<int:o_id>/copy_order/<int:p_id>/<string:edit_order>/', methods=['GET', ])
@login_required
@user_activated
...
@linen.route('/preprocess_order/', defaults={'o_id': None}, methods=['POST', ])
@linen.route('/preprocess_order/<int:o_id>', defaults={'p_id': None}, methods=['POST', ])
@linen.route('/preprocess_order/<int:o_id>/<int:p_id>', methods=['POST', ])
@login_required
@user_activated
def preprocess_order(o_id: int = None, p_id: int = None):
    return preprocess_order_category(o_id=o_id, p_id=p_id, category=settings.Linen.CATEGORY)


@linen.route('<int:o_id>/delete_order/<int:l_id>', defaults={'async_type': None}, methods=['POST', ])
@linen.route('<int:o_id>/delete_order/<int:l_id>/<int:async_type>', methods=['POST', ])
@login_required
@user_activated
def delete_order_pos(o_id: int, l_id: int, async_type: int = None):
    return helper_delete_order_pos(o_id=o_id, m_id=l_id, async_type=async_type, category=settings.Linen.CATEGORY,
                                   model=Linen)


@linen.route('<int:o_id>/clean_orders/', methods=['GET', ])
@login_required
@user_activated
...
def search_by_article(o_id: int):
    return h_category_sba(u_id=current_user.id, o_id=o_id, model_c=Linen, category=settings.Linen.CATEGORY_PROCESS)


@linen.route('/upload', methods=['GET', ])
@login_required
@user_activated
def upload():
    return helper_upload_common_get(category=settings.Linen.CATEGORY,
                                    category_process_name=settings.Linen.CATEGORY_PROCESS)


@linen.route('/process_upload', methods=['POST', ])
@login_required
def process_upload():
    return helper_upload_common_post(category=settings.Linen.CATEGORY,
                                     category_process_name=settings.Linen.CATEGORY_PROCESS,
                                     upload_model=UploadLinen)


@linen.route('/preload/<int:o_id>/<int:stage>', methods=['GET', ])
@user_activated
@login_required
def preload(o_id: int, stage: int):

    return helper_preload_common(o_id=o_id, stage=stage, category=settings.Linen.CATEGORY,
                                 category_process_name=settings.Linen.CATEGORY_PROCESS)
