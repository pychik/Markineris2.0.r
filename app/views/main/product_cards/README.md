...
- Списки стран в карточках берутся из общих справочников Redis/Tezaurus. Для `clothes`, `shoes`, `linen`, `socks` и `parfum` обычный список идет через `get_all_countries()`, список режима РД - через `get_rd_countries(category)`.
...
- Для карточек `cosmetics` и `toys` списки стран остаются как в обычных заказах этих категорий: без РД берутся `default_countries` конкретной подкатегории, при включенном РД - полный `get_all_countries()`.
