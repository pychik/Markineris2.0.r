...
class Settings(BaseSettings):
    ...
    CLOTHES_COUNTRIES_RD: list = CLOTHES_COUNTRIES_RD
    SHOES_COUNTRIES_RD: list = SHOES_COUNTRIES_RD
    PARFUM_COUNTRIES_RD: list = PARFUM_COUNTRIES_RD
    LINEN_COUNTRIES_RD: list = LINEN_COUNTRIES_RD
    ...
    FSA_TIMEOUT: float = float(os.getenv('FSA_TIMEOUT', '30'))
    ...
    RD_TYPES: tuple = ('Декларация соответствия', 'Сертификат',)
    ...
    TRUST_LIMIT_MINIMUM: int = 0
    TRUST_LIMIT_MAXIMUM: int = 1200000
    ...
    class OrderStage:
        ...
        AUTO_HOURS_CP: int = 148  # quantity of hours before changing order stage from MANAGER PROBLEM TO CANCEL
    ...
    class Prices:
        F_LTE_100: Decimal = Decimal('16.2')
        F_100_500: Decimal = Decimal('14.4')
        F_500_1K: Decimal = Decimal('8.1')
        F_1K_3K: Decimal = Decimal('6.3')
        F_3K_5K: Decimal = Decimal('5.4')
        F_5K_10K: Decimal = Decimal('5.0')
        F_10K_20K: Decimal = Decimal('4.4')
        F_20K_35K: Decimal = Decimal('3.6')
        F_35K_50K: Decimal = Decimal('3.1')
        F_50K_100K: Decimal = Decimal('2.7')
        F_100K: Decimal = Decimal('2.0')
        BASIC_PRICES: tuple = (
    ...
    class Messages:
        ...
        PROCESS_SUCCESS: str = "Заказ оформлен. Проверяйте статус в истории заказов. " \
                                  "Номер заказа "
        ...
        UPLOAD_MARK_TYPE_ERROR: str = "Произошла ошибка заполнения заказа. Не указан тип маркировки. "
        UPLOAD_FILE_EXTENSION_ERROR: str = "Загружаемый файл не соответствует шаблону," \
                                           " Скачайте шаблон и скопируйте в него данные из вашего файла!"
        UPLOAD_FILE_VALIDATION_ERROR: str = "В вашем файле присутствуют не прошедшие валидацию параметры" \
                                            " Скачайте шаблон и скопируйте в него данные из вашего файла!"
        UPLOAD_FILE_POS_ERROR: str = "В вашем заказе отсутствуют позиции!" \
                                 "Скачайте шаблон и скопируйте в него данные из вашего файла!"
        ...
        TNVED_ABSENCE_ERROR: str = "Проверьте что корректно заполняете ТНВЭД. Ваш тнвед либо пустой, либо не подходит типу одежды"
        COLOR_INPUT_ERROR: str = "Ошибка цвета. Система Честный знак не принимает цвет {color}"
        ...
        ORDER_MANAGER_PROCESSED_ABS_PROCESSING_INFO: str = "Во время переноса заказа в готовые произошла ошибка. Не указан УПД и организация! Нажмите на иконку УПД и укажите необходимую информацию!"
        ...
        ORDER_MANAGER_FON: str = "Во время привязки файла к заказу произошла ошибка. Можно прикреплять только архивы с наименованиями совпадающими с номером заказа. Ваш файл-"
        ...
        EDIT_PRICE: str = "Успешно отредактирован ценовой пакет {price_code}"
        EDIT_NE_PRICE: str = "Попытка редактирования несуществующего ценового пакета "
        EDIT_PRICE_ERROR: str = "Во время редактирования ценового пакета произошла ошибка!"
    ...
    class Reports:
        ...
        UT_START_ELSE: list = UT_REPORT_START_ELSE
    ...
    class Parfum:
        ...
        UPLOAD_SIZE_ERROR: str = "Проверьте правильность указанного размера "
    ...
    class Clothes:
        ...
        ALL_TYPES_WITH_SUBCATEGORIES: list = CLOTHES_TYPES + UNDERWEAR_TYPES + SWIMMING_ACCESSORIES_TYPES + HATS_TYPES + GLOVES_TYPES + SHAWLS_TYPES
        ...
        CLOTHES_SIZE_TYPES: list = CLOTHES_SIZE_TYPES
        ...
        UPLOAD_SIZE_ERROR: str = "Проверьте правильность указанного размера одежды"
    ...
    class Socks:
        ...
        UPLOAD_TYPE_ERROR: str = "Проверьте правильность выбора типа чулочно-носочных изделий (посмотрите вкладку справочник)"
        UPLOAD_COLOR_ERROR: str = "Проверьте правильность указанного цвета чулочно-носочных изделий (посмотрите вкладку справочник)"
        UPLOAD_SIZE_TYPE_ERROR: str = "Проверьте правильность указанного типа размера чулочно-носочных изделий (справочник)"

        UPLOAD_SIZE_ERROR: str = "Проверьте правильность указанного размера чулочно-носочных изделий"

        UPLOAD_CONTENT_ERROR: str = "Проверьте правильность указанного состава чулочно-носочных изделий (это не может быть цифра или пустое поле)"
        UPLOAD_GENDER_ERROR: str = "Проверьте правильность указанного пола (посмотрите вкладку справочник)"
        UPLOAD_QUANTITY_ERROR: str = "Проверьте правильность указанного количества (Это должно быть число )"
        UPLOAD_COUNTRY_ERROR: str = "Проверьте правильность указанной страны (посмотрите вкладку справочник)"
